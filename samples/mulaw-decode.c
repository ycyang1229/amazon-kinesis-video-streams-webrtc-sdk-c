...
static GstCaps* mulawdec_getcaps(GstPad* pad)
{
    ...
    if (pad == mulawdec->sinkpad) {
        ...
        base_caps = gst_caps_new_simple("audio/x-raw-int", "width", G_TYPE_INT, 16, "depth", G_TYPE_INT, 16, "signed", G_TYPE_BOOLEAN, TRUE,
                                        "endianness", G_TYPE_INT, G_BYTE_ORDER, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
    }
...
static GstPadLinkReturn mulawdec_link(GstPad* pad, const GstCaps* caps)
{
    GstMuLawDec* mulawdec = GST_MULAWDEC(gst_pad_get_parent(pad));
    GstPad* otherpad;
    GstStructure* structure;
    ...
    if (pad == mulawdec->srcpad) {
        ...
        otherpad = mulawdec->srcpad;
        base_caps = gst_caps_new_simple("audio/x-raw-int", "width", G_TYPE_INT, 16, "depth", G_TYPE_INT, 16, "endianness", G_TYPE_INT, G_BYTE_ORDER,
...
static void gst_mulawdec_base_init(GstMuLawDecClass* klass)
{
    ...
    GstElementDetails mulawdec_details = {"Mu Law to PCM conversion", "Codec/Decoder/Audio", "Convert 8bit mu law to 16bit PCM",
