...
static GstCaps* mulawenc_getcaps(GstPad* pad)
{
...
static GstPadLinkReturn mulawenc_link(GstPad* pad, const GstCaps* caps)
{
    GstMuLawEnc* mulawenc = GST_MULAWENC(gst_pad_get_parent(pad));
    GstPad* otherpad;
    GstStructure* structure;
...
static void gst_mulawenc_base_init(GstMuLawEncClass* klass)
{
    ...
    GstElementDetails mulawenc_details = {"PCM to Mu Law conversion", "Codec/Encoder/Audio", "Convert 16bit PCM to 8bit mu law",
