 ...
 */
static INT32 hexEncodedSha256(UINT8* pMsg, UINT32 uMsgLen, PCHAR pEncodedHash)
{
    SIZE_T i = 0;
    PCHAR p = NULL;
...
STATUS AwsSignerV4_addCanonicalHeader(AwsSignerV4Context_t* pCtx, PCHAR pHeader, UINT32 uHeaderLen, PCHAR pValue, UINT32 uValueLen)
{
    ...
    if (pCtx == NULL || pCtx->pBuf == NULL || pCtx->pBufIndex == NULL) {
        ...
        pCtx->pBufIndex += SPRINTF(pCtx->pBufIndex, CANONICAL_HEADER_TEMPLATE, (INT32) uHeaderLen, pHeader, (INT32) uValueLen, pValue);
...
STATUS AwsSignerV4_addCanonicalBody(AwsSignerV4Context_t* pCtx, UINT8* pBody, UINT32 uBodyLen)
{
    STATUS retStatus = STATUS_SUCCESS;
    INT32 xEncodedLen = 0;
                        ...
                        UINT32 uServiceLen, PCHAR pXAmzDate, UINT32 uXAmzDateLen)
{
    ...
    if (pCtx == NULL || pCtx->pBuf == NULL || pCtx->pBufIndex == NULL) {
        retStatus = STATUS_INVALID_ARG;
    } else if (pSecretKey == NULL || uSecretKeyLen == 0 || pRegion == NULL || uRegionLen == 0 || pService == NULL || uServiceLen == 0 ||
               pXAmzDate == NULL || uXAmzDateLen == 0) {
        retStatus = STATUS_INVALID_ARG;
    } else {
        ...
        if (retStatus >= 0) {
             ...
             */
            SNPRINTF(pCtx->pCredentialScope, MAX_SCOPE_LEN, CANONICAL_SCOPE_TEMPLATE, SIGNATURE_DATE_STRING_LEN, pXAmzDate, uRegionLen, pRegion,
                     uServiceLen, pService, AWS_SIG_V4_SIGNATURE_END);
             ...
             */
            xSignedStrLen =
                SPRINTF(pSignedStr, CANONICAL_SIGNED_TEMPLATE, AWS_SIG_V4_ALGORITHM, pXAmzDate, pCtx->pCredentialScope, pRequestHexSha256);
             ...
             *
             */
            SNPRINTF((PCHAR) pHmac, AWS_SIG_V4_MAX_HMAC_SIZE, SIGNATURE_START_TEMPLATE, AWS_SIG_V4_SIGNATURE_START, (INT32) uSecretKeyLen,
                     pSecretKey);

            uHmacSize = mbedtls_md_get_size(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256));

            if (mbedtls_md_hmac(md_info, pHmac, sizeof(AWS_SIG_V4_SIGNATURE_START) - 1 + uSecretKeyLen, (UINT8*) pXAmzDate, SIGNATURE_DATE_STRING_LEN,
                                pHmac) != 0) {
                ...
                retStatus = STATUS_AWS_SIGNER_FAIL_TO_CALCULATE_HASH;
            } else if (mbedtls_md_hmac(md_info, pHmac, uHmacSize, (UINT8*) AWS_SIG_V4_SIGNATURE_END, sizeof(AWS_SIG_V4_SIGNATURE_END) - 1, pHmac) !=
                       0) {
                retStatus = STATUS_AWS_SIGNER_FAIL_TO_CALCULATE_HASH;
            }
