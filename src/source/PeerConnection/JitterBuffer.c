                          ...
                          UINT32 maxLatency, UINT32 clockRate, UINT64 customData, PJitterBuffer* ppJitterBuffer)
{
    ...
    hashTableCreateWithParams(JITTER_BUFFER_HASH_TABLE_BUCKET_COUNT,
...
STATUS jitterBufferPush(PJitterBuffer pJitterBuffer, PRtpPacket pRtpPacket)
{
    ...
    if ((pRtpPacket->header.timestamp < pJitterBuffer->maxLatency && pJitterBuffer->lastPushTimestamp <= pJitterBuffer->maxLatency) ||
        ...
        if (STATUS_SUCCEEDED(status) && pCurPacket != NULL) {
            ...
            hashTableRemove(pJitterBuffer->pPkgBufferHashTable, pRtpPacket->header.sequenceNumber);
        }
        hashTablePut(pJitterBuffer->pPkgBufferHashTable, pRtpPacket->header.sequenceNumber, (UINT64) pRtpPacket);
...
STATUS jitterBufferPop(PJitterBuffer pJitterBuffer, BOOL bufferClosed)
{
    ...
    CHK(pJitterBuffer != NULL && pJitterBuffer->onFrameDroppedFn != NULL && pJitterBuffer->onFrameReadyFn != NULL,
        STATUS_NULL_ARG);
    ...
    for (; index != lastIndex; index++) {
        hashTableContains(pJitterBuffer->pPkgBufferHashTable, index, &hasEntry);
        if (!hasEntry) {
            isFrameDataContinuous = FALSE;
            CHK(pJitterBuffer->lastPopTimestamp < earliestTimestamp || bufferClosed, retStatus);
        } else {
            lastNonNullIndex = index;
            hashTableGet(pJitterBuffer->pPkgBufferHashTable, index, (PUINT64) &pCurPacket);
            ...
            CHK_STATUS(pJitterBuffer->depayPayloadFn(pCurPacket->payload, pCurPacket->payloadLength, NULL,
    ...
    if (bufferClosed && curFrameSize > 0) {
        curFrameSize = 0;
        for (index = startDropIndex; UINT16_DEC(index) != lastNonNullIndex; index++) {
            hashTableContains(pJitterBuffer->pPkgBufferHashTable, index, &hasEntry);
            if (hasEntry) {
                hashTableGet(pJitterBuffer->pPkgBufferHashTable, index, (PUINT64) &pCurPacket);
                CHK_STATUS(pJitterBuffer->depayPayloadFn(pCurPacket->payload,
                                                         pCurPacket->payloadLength, NULL,
...
STATUS jitterBufferDropBufferData(PJitterBuffer pJitterBuffer, UINT16 startIndex, UINT16 endIndex, UINT32 nextTimestamp)
{
    ...
    for (; UINT16_DEC(index) != endIndex; index++) {
        ...
        if (hasEntry) {
            hashTableGet(pJitterBuffer->pPkgBufferHashTable, index, (PUINT64) &pCurPacket);
            freeRtpPacket(&pCurPacket);
            hashTableRemove(pJitterBuffer->pPkgBufferHashTable, index);
