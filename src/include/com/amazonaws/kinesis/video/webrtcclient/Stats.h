...
typedef enum {
    ...
    */
    /**
     * Host Candidate: Host candidates are generated by the client by binding to its locally assigned IP addresses and port. 
     * If you have multiple IP addresses, you can generate multiple host candidates
    */
    ICE_CANDIDATE_TYPE_HOST = 0,             //!< ICE_CANDIDATE_TYPE_HOST, local.
    /**
     * As agents perform connectivity checks, they may produce additional candidates known as peer reflexive candidates. 
     * This usually happens when there is a symmetric NAT in between clients. 
     * During the connectivity check process, a STUN request is sent directly to the client, which can generate a brand new binding. 
     * If it does, the STUN response is sent back informing the originating client that a new binding was formed. 
     * This allows clients to have a direct media path between them, even in the presence of a symmetric NAT.
    */
    ICE_CANDIDATE_TYPE_PEER_REFLEXIVE = 1,   //!< ICE_CANDIDATE_TYPE_PEER_REFLEXIVE, remote.
    /**
     * Server Reflex Candidate: Server reflex candidates are generated by sending STUN messages to a STUN/TURN server. 
     * A client sends a query message to the STUN server. That query passes through the NAT which creates a binding. 
     * The response to the query contains the public IP and port that was generated for the binding. This can now be used as a server reflex candidate.
    */
    ICE_CANDIDATE_TYPE_SERVER_REFLEXIVE = 2, //!< ICE_CANDIDATE_TYPE_SERVER_REFLEXIVE, stun server.
    /**
     * Relay Candidate: Relay candidates are generated in the same way as a server reflex candidate. 
     * A query message is sent to the TURN server which creates a NAT binding. 
     * That binding is used, but the binding will be sent to and from the relay server.
    */
